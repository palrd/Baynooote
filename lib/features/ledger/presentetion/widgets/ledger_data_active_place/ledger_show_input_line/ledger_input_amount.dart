...
class _LedgerInputAmountState extends State<LedgerInputAmount>
  ...
  void initState() {
    ...
    vm.addListener(() {
      ...
      if (vm.amount == 0 && controller2.isCompleted) {
        controller2.reverse();
      }
      // ///意思是当金额从0变成大于0的时候才执行动画，因为后面在金额已经大于0的时候再怎么改变也不执行动画了
      else if (vm.amount != 0 && controller2.isDismissed) {
        controller2.forward();
      }
    });
  }
  ...
  void initAniamtion2() {
    controller2 = AnimationController(
      duration: const Duration(milliseconds: 800), // 调整波浪速度
      vsync: this,
    );
  ...
  Widget _buildShowAmount() {
    return Selector<RecordCollectionAmountViewModel, double>(
      builder: (_, amount, _) {
        return Text(
          amount.toString(),
          maxLines: 1,
